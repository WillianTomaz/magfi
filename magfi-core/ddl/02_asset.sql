...
CREATE INDEX idx_ticker_symbol ON dim_asset(ticker_symbol);
CREATE INDEX idx_asset_active ON dim_asset(is_active);
CREATE INDEX idx_asset_alert ON dim_asset(drop_alert_enabled, is_active);

-- Asset price history fact table
CREATE TABLE IF NOT EXISTS fct_asset_price_history (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_asset_history ON fct_asset_price_history(asset_id, recorded_at);
